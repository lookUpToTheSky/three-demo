class CanvasView {
    ...
    draw(obj) {
        ...
        ctx.lineWidth = obj.lineWidth = obj.lineWidth || '8';
    ...
    static type2(ctx, obj) {
        ...
        if(obj.borderColor.length < 0) {
            console.error('obj.borderColor is need a array');
            return false;
        }
    ...
    static type5(ctx, obj) {
        ...
        ctx.beginPath();
        ctx.arc(10, (obj.height-ctx.lineWidth*1)/3,ctx.lineWidth*2, 0, 2*Math.PI);
        ctx.fill();
        ctx.closePath();
        ctx.shadowBlur = 0;
        ctx.beginPath();
        ...
        ctx.shadowBlur = 10;
        ctx.beginPath();
        ctx.arc(obj.width/2, obj.height/1.5, ctx.lineWidth*2, 0, 2*Math.PI);
        ctx.fill();
        ctx.closePath();
    ...
    static type6(ctx, obj) {
        //文字框
        let box = [{point1:[obj.lineWidth/2, obj.height/2-20],point2:[obj.width-obj.lineWidth/2,obj.height/2-20],
            point3:[obj.width-obj.lineWidth/2, obj.height/2+20],point4:[obj.lineWidth/2, obj.height/2+20]}];
        ctx.lineWidth = '2';
        ctx.strokeStyle = obj.borderColor[0];
        box.forEach(item => {
            ctx.beginPath();
    ...
    static type7(ctx, obj) {
        ...
        ctx.fillStyle = obj.borderColor[1];
        ctx.beginPath();
        ctx.arc(22 , obj.height/2-obj.lineWidth/2, 10, 0, Math.PI*2);
        ctx.closePath();
        ctx.fill();
        CanvasView.drawText(ctx, obj);
    }
    ...
    static drawText(ctx, obj) {
        ...
        if(typeof(obj.title) == 'object') {
            ...
            titleTop = obj.title.top|| 5;
            titleLeft = obj.title.left|| 0;
        }else{
            title = obj.title;
            ctx.fillStyle = '#fff';
            titleTop = 5;
        ...
        if(title !== undefined) {
            ...
            ctx.beginPath();
            ctx.fillText(title, titleAlign+titleLeft, titleSize+titleTop)
            ctx.closePath();
            rowStart++;
        }
        //文本内容
