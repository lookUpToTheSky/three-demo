...
THREE.onEvent = function(scene,camera, width, height) {
	option.scene = scene || {};
...
listenerList.click = function (targetList,camera) {
	...
	function down(event) {
		...
		var list = [];
		Mouse.setFromCamera(new THREE.Vector2(( event.clientX / option.width ) * 2 - 1,- ( event.clientY / option.height ) * 2 + 1), camera);
		list = getObjList(targetList);
...
listenerList.hover = function (targetList,camera) {
	...
	window.addEventListener('mousemove',function(event) {
		...
		list = getObjList(targetList);
		var intersects = Mouse.intersectObjects(list);
	    if (intersects.length > 0&&intersects.length==last) {
	    	if (Hover) return;
	    	Hover = true;
	      	targetObject = intersects[0].object;
	      	obj = getEventObj(targetList,targetObject);
			  if(!!obj.callback[0]) obj.callback[0](targetObject);
			last = intersects.length;
	    } else {
